...
class _AdminChatScreenState extends State<AdminChatScreen> {
  ...
  void _selectChatRoom(String roomId, Map<String, dynamic> data) {
    setState(() {
      _selectedChatRoomId = roomId;
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _buildChatRoomTile(room.id, data, isMobile: true);
                  },
                );
              },
            ),
  ...
  Widget _buildMobileChatView() {
    final chatType = _selectedChatData?['type'] as String? ?? 'general';
    String typeLabel = chatType == 'disposal' ? '처분' : (chatType == 'relocation' ? '이전' : '일반');
                        ...
                        builder: (context, snapshot) {
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              final room = chatRooms[index];
                              final data = room.data() as Map<String, dynamic>;
                              return _buildChatRoomTile(room.id, data, isMobile: false);
                            },
                          );
                        },
                      ),
  ...
  // ==================== 공통 위젯 ====================
  Widget _buildChatRoomTile(String roomId, Map<String, dynamic> data, {required bool isMobile}) {
    ...
    return InkWell(
      onTap: () => _selectChatRoom(roomId, data),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        decoration: BoxDecoration(
          color: isSelected && !isMobile ? AppColors.primary.withValues(alpha: 0.1) : null,
          border: Border(
            bottom: BorderSide(color: AppColors.grey200),
            left: isSelected && !isMobile
